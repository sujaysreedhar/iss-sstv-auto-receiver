...
(
    crontab -l 2>/dev/null | grep -v "check_and_record.sh" | grep -v "update_tle.sh"
    echo "0 0 * * * $HOME/iss-sstv-auto-receiver/update_tle.sh >> $HOME/iss_sstv/tle_update.log 2>&1"
